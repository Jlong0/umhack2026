...
export default function AuditLogDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col border-l border-white/5 bg-gray-950 shadow-2xl transition-transform duration-300 ${
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-white/5 px-5 py-4">
          <div>
            <h2 className="text-base font-bold text-gray-100">Action Audit Log</h2>
            <p className="text-xs text-gray-500">{logEntries.length} entries recorded</p>
          </div>
        ...
        <div className="flex gap-2 border-b border-white/5 px-5 py-3">
          <select
            ...
            className="rounded-lg border border-white/10 bg-white/[0.02] px-2 py-1 text-xs text-gray-300"
          >
          ...
          <input
            ...
            className="flex-1 rounded-lg border border-white/10 bg-white/[0.02] px-3 py-1 text-xs text-gray-300 placeholder-gray-600"
          />
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            ...
            <div className="divide-y divide-white/[0.03]">
              {filtered.map((entry) => (
                <div key={entry.id} className="px-5 py-3 transition-colors hover:bg-white/[0.02]">
                  ...
                  {entry.workerId && (
                    <span className="mt-1 inline-block font-mono text-[10px] text-gray-600">
