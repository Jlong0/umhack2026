...
function ToolConfirmCard({ toolCall, onConfirm, onCancel }) {
  ...
  return (
    <div className="mt-2 rounded-xl border border-indigo-200 bg-indigo-50 p-3 text-sm">
      <p className="font-medium text-indigo-900">Action required</p>
      ...
      <div className="mt-2 flex gap-2">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 hover:border-rose-300 hover:text-rose-700 disabled:opacity-60 transition"
        >
...
function MessageBubble({ message, onConfirm, onCancel }) {
  ...
  return (
    <div className={`flex gap-2 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${isUser ? "bg-indigo-600" : "bg-slate-700"}`}>
...
export default function AICommandConsole() {
  ...
  return (
    <>
      ...
      {/* Chat panel */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 flex w-[420px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20">
          {/* Header */}
          ...
          <div className="border-t border-slate-100 p-3">
            <div className="flex items-end gap-2">
              <textarea
                ...
                rows={1}
                disabled={isThinking}
                className="flex-1 resize-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-300/40 disabled:opacity-50"
