...
export default function StatutoryExposureCalculator() {
  ...
  return (
    <div className={`permit-surface overflow-hidden ${
      expiredCount > 0 ? "border-red-200 bg-gradient-to-br from-card to-red-50 dark:border-red-900 dark:to-red-950/30" : ""
    }`}>
      {/* Risk Indicator Bar */}
      {expiredCount > 0 && (
      ...
      <div className="p-5">
        ...
        <div className="mt-3 flex items-baseline gap-2">
          <span className={`font-mono text-3xl font-black tracking-tight ${
            expiredCount > 0 ? "text-red-700 dark:text-red-400" : "text-emerald-700 dark:text-emerald-400"
          }`}>
            {expiredCount > 0 ? asCurrency(minExposure) : "RM 0"}
          </span>
          {expiredCount > 0 && (
            <span className="text-sm text-red-500 dark:text-red-400">minimum</span>
          )}
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="rounded-lg bg-muted p-3">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Expired Permits</p>
            <p className={`mt-1 font-mono text-xl font-bold ${
              expiredCount > 0 ? "text-red-700 dark:text-red-400" : "text-foreground"
            }`}>{expiredCount}</p>
          </div>
          <div className="rounded-lg bg-muted p-3">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Expiring (30d)</p>
            <p className={`mt-1 font-mono text-xl font-bold ${
              expiringCount > 0 ? "text-amber-700 dark:text-amber-400" : "text-foreground"
            }`}>{expiringCount}</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          {expiredCount === 0 ? (
            <>
              ...
              <span className="text-xs font-medium text-emerald-700 dark:text-emerald-400">All permits compliant</span>
            </>
          ) : (
            <>
